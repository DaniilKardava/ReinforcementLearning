...
class ConvPoolLayer:
    ...
    def forward(self, input):
        ...
        for feature_out in range(self.output_features):
            # Correlate input and weight kernel to form feature map. Compute channel by channel and sum.
            channels = []
            input_depth = input.shape[0]
            for channel in range(input_depth):
                feature_map = signal.correlate2d(
                    input[channel, :, :],
                    self.weights[feature_out, channel, :, :],
                    mode="valid",
                )
                channels.append(feature_map)

            feature_map = np.sum(channels, axis=0)
    ...
    def calculate_deltas(self, layer_error, prev_layer_nodes):
        ...
        for feature in range(output_features):
            ...
            for channel in range(kernel_depth):
                ...
                correl_input = prev_layer_nodes[channel, :, :]
